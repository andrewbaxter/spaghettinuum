...
pub fn ensure(db: &rusqlite::Connection) -> Result<(), GoodError> {
    let query =
...
pub fn get_cert(db: &rusqlite::Connection) -> Result<DbRes1, GoodError> {
    let query =
...
pub fn set_cert(db: &rusqlite::Connection, pub_: Option<&str>, priv_: Option<&str>) -> Result<(), GoodError> {
