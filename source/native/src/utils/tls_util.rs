...
pub fn cert_der_hash(cert_der: &[u8]) -> Result<Vec<u8>, loga::Error> {
    return Ok(
...
pub fn cert_pem_hash(cert_pem: &str) -> Result<Vec<u8>, loga::Error> {
    return Ok(
        <Sha256 as Digest>::digest(
...
where
    ...
    S2F: Future<Output = Result<Vec<u8>, loga::Error>>,
    S2: FnOnce(Vec<u8>) -> S2F {
    ...
    let signature = issuer_signer2(Vec::<u8>::from(csr_der)).await?;
    return Ok(
        cert_builder
    ...
    return Ok(
        create_leaf_cert_der(
            ...
            key.clone(),
            {
                let mut key = key;
                move |csr| async move {
                    return Ok(SignerMut::<DerSignature>::try_sign(&mut key, &csr)?.to_bytes().to_vec());
                }
            },
            issuer_fqdn,
        ).await?,
