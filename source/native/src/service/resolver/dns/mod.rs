...
pub async fn start_dns_bridge(
    ...
    let dot_bind_addrs = if let Some(bind_addrs) = dns_config.dot_bind_addrs {
        let mut out = vec![];
        for bind_addr in bind_addrs {
            out.push(bind_addr.resolve()?);
        }
    ...
    for bind_addr in dot_bind_addrs {
        server
    ...
    if !registered {
        return Err(loga::err("No UDP/TCP or DOT bind addresses defined for DNS resolver"));
    }
